...
function createLoaderTask$1(router, lane, index, tasks, semanticParent, options, retainedEnd) {
	...
	const outcome = (reloadFailure ? Promise.resolve(reloadFailure) : !blocking ? Promise.resolve([SUCCESS$1, match.loaderData]) : loadResource(router, lane, match, route, loader, semanticParent, preload, options[0])).then((result) => {
		if (blocking) {
			...
			if (result[0] === SUCCESS$1) {
				if (routeLoader && !options[0].signal.aborted && !(!preload && router._tx?.[6])) cacheLoaderMatch(router, match, plannedCacheMatch);
...
async function runBackground(router, tx, base, tasks, settlement) {
	...
	if (isControl(reduced)) {
		transferMatchResources(router, next);
		if (reduced[0] === REDIRECTED$1 && router._tx === tx && router._committed === base) await followRedirect(router, tx, reduced[1]);
		return;
	}
...
async function runClientTransaction(router, tx, forceStaleReload, onReady, sync, resolvedPrefix) {
	...
	if (isControl(result)) {
		if (result[0] === REDIRECTED$1 && router._tx === tx) {
			finishPending(router, tx);
			transferMatchResources(router, tx[3]);
			tx[3] = [];
			if (router._tx === tx) {
	...
	if (pending?.[0] === tx) {
		...
		*/
		...
		if (pending[4]) {
			...
			let rendered = false;
			try {
				rendered = await waitFor$1(pending[4], signal);
			} catch (cause) {
				if (cause !== signal) throw cause;
			}
